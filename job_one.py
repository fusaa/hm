...
def main():
    ...
    # product_id_from_showcase = ['0811993034']  # for checking on specific pages instead all
    print(product_id_from_showcase)
...
def get_more_selenium(product_id, current_showcase_id):
    ...
    description = soup.find('p', class_='BodyText-module--general__32l6J ProductDescription-module--descriptionText__1zy9P BodyText-module--large__1RR86')
...
def get_colors_ids_from_prod_page(page_source):
    ...
    try:
        ...
        # filter-option miniature unavailable active
        # sometimes the showcase item might be unavailable, so the class will be different
        print('COLOR LIST EXTRACTION OF ACTIVE COLOR FAILLED')
        color_list_soup.append(soup.find_all('a', class_='filter-option miniature unavailable active')[0])
